...
def create_reference_dbs_router(reference_db_service=None):
    ...
    @router.post("")
    def create_reference_db(payload: dict):
    ...
    @router.get("")
    def list_reference_dbs(user_id: str):
        if reference_db_service is None:
            raise HTTPException(status_code=status.HTTP_503_SERVICE_UNAVAILABLE, detail="Reference DB service unavailable")
        return [
            {
            ...
            for item in reference_db_service.list_owned(user_id=user_id)
        ]

    @router.delete("/{reference_db_id}")
    def delete_reference_db(reference_db_id: str, user_id: str):
        if reference_db_service is None:
            raise HTTPException(status_code=status.HTTP_503_SERVICE_UNAVAILABLE, detail="Reference DB service unavailable")
        try:
            reference_db_service.delete_owned(user_id=user_id, reference_db_id=reference_db_id)
