...
def create_tasks_router(task_service=None):
    ...
    @router.post("")
    def create_task(payload: dict, x_correlation_id: str = Header(default="missing-correlation-id")):
    ...
    def cancel_task(task_id: str, payload: dict, x_correlation_id: str = Header(default="missing-correlation-id")):
        if task_service is None:
            raise HTTPException(status_code=status.HTTP_503_SERVICE_UNAVAILABLE, detail="Tasks unavailable")
